...
// ******* start validation ********************************
router.get('/release1-v2/pre-one-login/start', function (req, res) {
  // Set URl
  res.render('release1-v2/pre-one-login/start', {
    currentUrl: req.originalUrl
  })
})

router.post('/release1-v2/pre-one-login/start', function (req, res) {
  if (req.session.data['version'] === 'sole-trader') {
    // ACSP
    res.redirect('/release1-v2/pre-one-login/one-login-intro')
  } else {
    // General
    res.redirect('/release1-v2/pre-one-login/create-or-sign-in')
  }
})
